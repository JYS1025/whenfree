...
const translations: Record<Language, Translations> = {
  ko: {
    ...
    // Home
    platformBadge: "Agent-Native Scheduling Platform",
    homeTitle: "새로운 모임 일정 잡기",
    homeDesc: "시간표를 일일이 확인할 필요 없이, 생성된 링크를 AI 비서(ChatGPT/Claude)에게 전달하면 캘린더를 확인하고 일정을 등록해 줍니다.",
    ...
    aiHeroTitle: "AI 비서에게 캘린더 일정 등록 위임하기",
    ...
    aiCopyButton: "AI 지침 프롬프트 복사 (ChatGPT / Claude용)",
    aiCopiedSuccess: "프롬프트가 복사되었습니다. ChatGPT나 Claude에 붙여넣으세요 (Ctrl+V)",
    ...
    savingButton: "저장 중...",
    savedSuccess: "일정이 저장되었습니다.",
    timeCol: "시간",

    // Grid - Right (Group)
    groupTitle: "모두의 현황",
    groupSubtitle: "시간표 위에 마우스를 올리면 가능한 사람이 표시됩니다.",
    hoverPrompt: "시간표 칸 위로 마우스를 올리면 참석 가능 현황이 표시됩니다.",
  ...
  en: {
    ...
    aiCopyButton: "Copy AI Instructions (Paste into ChatGPT / Claude)",
...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("whenfree_lang") as Language;
  ...
  const setLang = (newLang: Language) => {
    ...
    localStorage.setItem("whenfree_lang", newLang);
  };
